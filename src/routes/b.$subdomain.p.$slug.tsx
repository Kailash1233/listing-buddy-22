...
export const Route = createFileRoute("/b/$subdomain/p/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    const description = (
      ...
      `${bits.join(" · ")}. ${property.listing_type === "rent" ? "For rent" : "For sale"} in ${property.city ?? "Chennai"}. Enquire on WhatsApp.`
    ).slice(0, 200);
    const cover = photoPaths(property.photos)[0];
    // `.og.jpg` is a 1200x630 crop generated by the media route — small enough
    // for WhatsApp/Twitter previews, unlike the multi-MB original.
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <main className="mx-auto max-w-3xl px-4">
        ...
        <p className="mt-8 text-xs leading-relaxed text-muted-foreground">
          ...
          Listing details are provided by {broker.name} and are not verified by this platform.
          Please verify documents before any transaction.
        </p>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-card/95 p-3 backdrop-blur">
        <div className="mx-auto flex max-w-3xl gap-2">
          <Button asChild className="flex-1" onClick={() => track("whatsapp_click")}>
            <a href={waLink(broker.whatsapp_number, waMessage)} target="_blank" rel="noreferrer">
...
function LeadForm({ property }: { property: Property }) {
  ...
  if (done) {
    return (
      <section
        id="enquiry"
        className="surface mt-8 flex flex-col items-center gap-2 p-8 text-center"
      >
        <ShieldCheck className="size-8 text-primary" />
        <p className="font-semibold">Enquiry sent!</p>
        <p className="text-sm text-muted-foreground">
          The broker will get in touch with you shortly.
        </p>
      </section>
    );
  }
  ...
  return (
    <section id="enquiry" className="surface mt-8 space-y-4 p-5">
      ...
      {/* Honeypot: hidden from real visitors, only a bot auto-filling every field on the form would set this. */}
      <div
        aria-hidden="true"
